...
except ImportError:  # pragma: no cover - optional dependency for Reformer only
    LSHSelfAttention = None

try:
    from einops import rearrange
except ImportError:  # pragma: no cover - optional dependency for FlashAttention only
    rearrange = None
...
class FlashAttention(nn.Module):
    ...
    def flash_attention_forward(self, Q, K, V, mask=None):
        if rearrange is None:
            raise ImportError(
                "FlashAttention requires `einops`, which is not needed for vanilla iTransformer."
            )
        BLOCK_SIZE = 32
...
class ReformerLayer(nn.Module):
    def __init__(self, attention, d_model, n_heads, d_keys=None,
        ...
        if LSHSelfAttention is None:
            raise ImportError(
                "ReformerLayer requires `reformer_pytorch`, which is not needed for vanilla iTransformer."
            )
